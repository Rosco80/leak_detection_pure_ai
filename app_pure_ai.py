...
@st.cache_resource
def load_model():
    """Load pre-trained ensemble model (XGBoost + Random Forest)"""
    try:
        model_path = 'leak_detector_ensemble.pkl'
        scaler_path = 'feature_scaler_ensemble.pkl'
...
# Header
st.markdown('<div class="main-header">🔧 Valve Leak Detection System - Pure AI Ensemble</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">XGBoost + Random Forest | 4-Week Pilot Demo</div>', unsafe_allow_html=True)
...
with st.expander("ℹ️ About This Demo", expanded=False):
    ...
    This system demonstrates **pure AI ensemble learning** for valve leak detection using acoustic emission (AE) sensor data.
    ...
    **How it works:**
    ...
    3. **AI Ensemble analyzes the data:**
       - 🤖 **XGBoost** (60% weight): Gradient boosting, handles imbalanced data
       - 🌲 **Random Forest** (40% weight): Bagging ensemble, robust to overfitting
    4. **Soft voting**: Averages predicted probabilities from both models

    **System Performance:**
    - Detects known leaks: C402 at 89.4%, 578-B at 89.3%
    - Training: 69 valves (23 leak via SMOTE, 46 normal)
    - Features: 17 total - AI learns optimal pattern thresholds from data
    - Training accuracy: 100% (with data augmentation)

    **Key Advantages:**
    - ✅ **Pure Machine Learning**: No hard-coded rules (e.g., continuity > 0.55)
    - ✅ **Ensemble Power**: Combines strengths of two different algorithms
    - ✅ **Better Performance**: 89% vs 59% on known leaks
    - ✅ **Scalable**: Easy to retrain with new data, add more models

    **Note:** This is the production AI system for the 4-week pilot. AI learns pattern detection thresholds directly from training data.
    """)
        ...
        with st.spinner("Extracting features from all cylinders and running AI model..."):
            ...
            if all_cylinder_data is None or len(all_cylinder_data) == 0:
                ...
                if model is None:
                    ...
                    # Make predictions for each valve using PURE AI ENSEMBLE
                    for valve_data in all_cylinder_data:
                        features = valve_data['features']
                        feature_df = pd.DataFrame([features])
                        feature_scaled = scaler.transform(feature_df)

                        # Pure AI Ensemble Prediction (XGBoost + Random Forest)
                    ...
                    for cyl_num in sorted(cylinders.keys()):
                        ...
                        # Create table for this cylinder's valves (PURE AI RESULTS)
                        valve_results = []
                        for valve in valves:
                            status_display = "⚠️ LEAK" if valve['prediction'] == 1 else "✅ Normal"

                            valve_results.append({
                                "Valve Position": valve['valve_name'],
                                "Status": status_display,
                                "AI Probability": f"{valve['leak_probability']:.1f}%",
                    ...
                    # Technical details
                    with st.expander("🔬 Technical Details - Pure AI Ensemble System"):
                        st.markdown("""
                        **🎯 Detection Approach: Pure AI Ensemble (XGBoost + Random Forest)**

                        This system uses a **voting ensemble** of two machine learning algorithms trained on 17 features.

                        **1. 🤖 XGBoost Classifier (60% weight)**
                        - **Algorithm:** Gradient Boosting Decision Trees
                        - **Why:** Excellent for imbalanced data, learns non-linear patterns
                        - **Parameters:**
                          - n_estimators: 100 trees
                          - max_depth: 6
                          - scale_pos_weight: 5.75 (handles 8:46 class imbalance)
                          - subsample: 0.8
                          - colsample_bytree: 0.8
                        - **Strength:** Handles minority class (leaks) effectively

                        **2. 🌲 Random Forest Classifier (40% weight)**
                        - **Algorithm:** Bootstrap Aggregating (Bagging) of Decision Trees
                        - **Why:** Robust to overfitting, provides stable predictions
                        - **Parameters:**
                          - n_estimators: 100 trees
                          - max_depth: 10
                          - class_weight: balanced
                        - **Strength:** Reduces variance, generalizes well

                        **3. ✅ Ensemble Strategy**
                        - **Voting Type:** Soft voting (probability averaging)
                        - **Weights:** 0.6 × XGBoost + 0.4 × Random Forest
                        - **Decision:** Leak if ensemble probability ≥ 50%
                        - **Result:** Combines strengths of both algorithms
                        ...
                        **Key Difference from Hybrid System:**
                        - ❌ **No hard-coded rules** (e.g., continuity > 0.55)
                        - ✅ **AI learns optimal thresholds** from training data
                        - ✅ **Pattern detection is learned**, not manually programmed

                        **Training Data:**
                        - **Total:** 69 valves (23 leak via SMOTE, 46 normal)
                        - **Augmentation:** SMOTE synthetic oversampling (8 → 23 leak samples)
                        - **Training Accuracy:** 100% (with augmentation)

                        **Performance on Known Leaks:**
                        - **C402 Cyl 3 CD:** 89.4% (↑ from 59.4% with single RF)
                        - **578-B Cyl 3:** 89.3% (↑ from 70.1% with single RF)
                        - **Improvement:** +30% better leak detection confidence

                        **Why Pure AI is Better:**
                        1. **No Manual Tuning:** AI discovers optimal decision boundaries
                        2. **Ensemble Robustness:** Two algorithms vote, reducing errors
                        3. **Better Imbalance Handling:** XGBoost + SMOTE handle rare leaks
                        4. **Scalable:** Easy to retrain with new data, add more models
                        """)
